context = (function () {
  ...
  function initialize(opts) {
    ...
    $(document).on('click', function (e) {
      ...
      else if(!$(e.target).parents(opts.manualClose).length){
        hide();
      }
    });
    if(options.preventDoubleContext){
      $(document).on('contextmenu click.contextmenu', '.dropdown-context', function (e) {
        e.preventDefault();
      });
    }
  ...
  function addContext(selector, data, mouseButton) {
    ...
    var trigger = '';
    if (mouseButton.indexOf('right') > -1) {
      trigger += 'contextmenu';
    }
    if (mouseButton.indexOf('left') > -1) {
      trigger += ' click.contextmenu';
    }
    $(selector).on(trigger, selector, function (e) {
      ...
      $dd = $('#dropdown-' + id);
      if(data.dynamic){
        $dd.children()[data.dynamic.index].childNodes[0].childNodes[1].data = data.dynamic.getText()
      }
    ...
    if (typeof data.scrollDetectors !== 'undefined' && data.scrollDetectors.length > 0) {
      for (var i in data.scrollDetectors) {
        $(data.scrollDetectors[i]).scroll(function () {
          hide();
        })
      }
    }
  ...
  function destroyContext(selector) {
    $(document).off('contextmenu', selector).off('click.contextmenu', selector).off('click', '.context-event');
  }
...
var createCallback = function(func, e) {
  return function(event) { func(event, currentContextSelector, e) };
};
